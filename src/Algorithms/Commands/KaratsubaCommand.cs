...
namespace Algorithms.Commands
{
    public class KaratsubaCommand : ICommand<KaratsubaOptions>
    {
        ...
        public KaratsubaCommand(TextWriter consoleOut)
        {
        ...
        public void Execute(KaratsubaOptions options)
        {
            ...
            var product = string.Concat(MultiplyXAndY(x, y).ToArray().Select(n => n.ToString()));
            _consoleOut.WriteLine(product);
        }

        private Span<int> MultiplyXAndY(Span<int> x, Span<int> y)
        {
        ...
        private Span<int> GetRecursiveCase(Span<int> x, Span<int> y)
        {
            ...
            return ac.Concat(new int[x.Length].AsSpan())
